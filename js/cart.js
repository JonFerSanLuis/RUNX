...
async function cartDetails(){
  const items=await Promise.all(getCart().map(async item=>{
    try{return {...item,product:await getProduct(item.productId)};}catch(error){return null;}
  }));
  return items.filter(Boolean);
}
function cartSubtotal(items){return items.reduce((sum,item)=>sum+item.product.price*item.quantity,0);}
...
async function renderCart(){
  ...
  if(!items.length){target.innerHTML=getCart().length?`<div class="empty-state"><h2>No hemos podido cargar tu carrito.</h2><p class="text-secondary">Inténtalo de nuevo cuando el catálogo esté disponible.</p><a href="carrito.html" class="btn btn-primary">Reintentar</a></div>`:`<div class="empty-state"><h2>Tu carrito está vacío</h2><p class="text-secondary">Añade productos de la colección para verlos aquí.</p><a href="tienda.html" class="btn btn-primary">Ir a la tienda</a></div>`;return;}
...
async function renderCheckoutSummary(){
  ...
  const subtotal=cartSubtotal(items);
  target.innerHTML=`<div class="checkout-summary d-lg-none"><details><summary>Ver resumen del pedido (${items.reduce((sum,item)=>sum+item.quantity,0)})</summary><div>${products}${summaryMarkup(subtotal,true)}</div></details></div><div class="d-none d-lg-block">${products}${summaryMarkup(subtotal,true)}</div>`;
}
